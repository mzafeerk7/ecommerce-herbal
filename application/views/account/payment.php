<?php
	...
	// invoice amount - 2.21 USD; or you can use - $amountUSD = convert_currency_live("EUR", "USD", 22.37); // convert 22.37EUR to USD
	if(isset($order_detail)){
		$orderID   = $order_detail[0]['order_id'];
		$amountUSD = $order_detail[0]['order_total_amount'];
	}else{
		exit();
	}
	...
	$all_keys = array(	"bitcoin"   => array("public_key" => "50607AAlhHnnBitcoin77BTCPUBKzS86HUzlAyz2lYEjzd6Qm6",  
										    "private_key" => "50607AAlhHnnBitcoin77BTCPRVr16Jbxix665lhw0Q9UR2avC"));
...
<html lang="en">
  ...
  <?php
	...
	$custom_text  = "";
	$custom_text .= "<a href='".base_url()."' >Home</a>";
	$custom_text .= "<br>";
	$custom_text .= "<p style='text-align: left !important;'><span >Order ID # ".prefix_zero($orderID, 6)."</span><p>";
	$custom_text .= "<p style='text-align: left !important;'><span class='pull-right'>Total : $".$amountUSD."</span><p>";
     
	$logo_path = base_url().'assets/img/logo/logo.png';
    //echo $box->display_cryptobox_bootstrap($coins, $def_coin, $def_language, $custom_text, 50, 100, false, $logo_path, "default", 250, "", "curl", false);
	//echo $box->display_cryptobox_bootstrap($coins, $def_coin, $def_language, $custom_text, 71, 100, false, "", "default", 200, "", "curl", false);
	echo $box->display_cryptobox_bootstrap($coins, $def_coin, $def_language, $custom_text, 50, 100, false, $logo_path, "default", 250, "", "ajax", false);
   ...
   <div class="text-center mt-0 mb-5">
	<a href="<?= base_url().'account/payment-process' ?>" class="btn btn-primary btn-lg"> Click here if you have sent Bitcoins </a>
   </div>
